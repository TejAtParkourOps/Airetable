...
import { getListOfBases, getListOfRecordsInTable, getListOfTablesInBase } from "./integrations/airtable"
import { AirtableField, AirtableRecord, AirtableTable } from "./integrations/airtable/responseTypes"
import { ABase, AField, ATable, ARecord, ACell } from "@common/airtableResource"
...
const processRecords = (rawRecords: Array<AirtableRecord>, fields: Record<string, AField>) : Record<string, ARecord> => {
...
> = (server, socket) => async (
    ...
    try {
        ...
        if (!_base) {
            return cb(makeErrorResponse(404, `Could not find Airtable Base: '${data.baseId}'`));
        }
        // get tables in base
        const _tables = (await getListOfTablesInBase(data.authToken, data.baseId)).tables;
        // get records for each table in base
        const recordsByTable : Record<string, Array<AirtableRecord>> = {};        
        for (const t of _tables){
            recordsByTable[t.id] = (await getListOfRecordsInTable(data.authToken, data.baseId, t.id)).records
        }
        // populate base
        ...
        if (err instanceof AxiosError) {
            switch (err.response?.status) {
                case 400: // this is probably because of something internal
                    return cb(makeErrorResponse(500, "Something went wrong while trying to fetch Airtable Base."));
                case 401: 
                    return cb(makeErrorResponse(401, "Your Airtable authorization token does not have access to the requested Airtable Base."));
                case 404:
                    return cb(makeErrorResponse(404, `Could not find Airtable resource!`));
                default:
            }
        }
        // handle other errors
        cb(makeErrorResponse(500, "Something went wrong while trying to fetch Airtable Base."));
    }
}
